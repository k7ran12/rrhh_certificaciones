    ...
    <div class="row align-items-center justify-content-center text-start">
                ...
                <form method="POST" action="{{ route('login') }}" class="needs-validation" novalidate="" id="form_data">
                    ...
                    <div>
                        <div class="form-group mb-3">
                            ...
                            <input class="form-control @error('email') is-invalid @enderror" id="email" type="email" name="email" value="{{ old('email') }}" placeholder="Ingrese su Email" required autocomplete="email" autofocus>
                            @error('email')
                        ...
                        <div class="d-grid">
                            <button style="background-color: #27282d !important;border-color: #202125 !important;" type="submit" class="login-do-btn btn btn-primary btn-block mt-2" tabindex="4">{{ __('Login') }}</button>
                        </div>

                        @if(Utility::getValByName('disable_signup_button')=='on')
                        <p class="my-4 text-center">{{ __("No tiene cuenta?") }}
                            <a href="{{route('register',$lang)}}" class="my-4 text-primary">{{__('Registro')}}</a>
                        </p>
                        @endif
                    </div>
                </form>
        ...
        <div class="col-xl-6 img-card-side">
            <div class="auth-img-content">
                <img src="{{ asset('assets/images/auth/logo_atenea.svg') }}" alt="" class="img-fluid mx-auto d-block">
            </div>
        </div>
    </div>
</div>
